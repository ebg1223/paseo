...
export interface PiImportSessionHookInput {
...
export interface PiDialect {
  ...
  resolveLaunchMode?: (modeId: string | undefined) => {
    modeId: string;
    extraArgs?: string[];
  };
...
const PI_DIALECT: PiDialect = {
  providerId: PI_PROVIDER,
  label: "Pi",
  defaultCommand: [PI_BINARY_COMMAND],
  commandsRpcName: "get_commands",
};

function resolveDialectLaunchMode(
  dialect: PiDialect,
  requestedModeId: string | undefined,
): { modeId: string | null; extraArgs?: string[] } {
  if (!dialect.resolveLaunchMode) {
    return { modeId: null };
  }
  const resolved = dialect.resolveLaunchMode(requestedModeId ?? dialect.defaultModeId);
  return {
...
function capabilitiesForClient(dialect: PiDialect): AgentCapabilityFlags {
  return withPiCapabilities(dialect.supportsMcpServers === true);
}

function capabilitiesForSession(dialect: PiDialect, hasMcpConfig: boolean): AgentCapabilityFlags {
  return withPiCapabilities(dialect.supportsMcpServers === true || hasMcpConfig);
}
...
function createPiPaseoExtensionFile(): PiTempFile {
  // D3(omp-native-branch): move this injected extension bridge back to the Pi adapter
  // when OMP switches to native branch/session RPCs.
...
function createRuntime(
  ...
  return new PiCliRuntime({
    logger,
    runtimeSettings,
    command: dialect.defaultCommand,
    commandsRpcName: dialect.commandsRpcName,
  });
}
...
export class PiRpcAgentSession implements AgentSession {
  ...
  constructor(options: PiRpcAgentSessionOptions) {
    ...
    this.provider = options.dialect.providerId;
    ...
    options.dialect.onSessionStart?.(this.runtimeSession, this.logger);
  }

  private readonly runtimeSession: PiRuntimeSession;
  private readonly config: AgentSessionConfig;
  private readonly dialect: PiDialect;
  ...
  async setMode(modeId: string): Promise<void | AgentProviderNotice> {
    const modes = this.dialect.modes;
    if (modes) {
      if (!modes.some((mode) => mode.id === modeId)) {
        throw new Error(`Invalid ${this.dialect.label} mode '${modeId}'`);
      }
      return this.dialect.setModeNotice?.(modeId);
    }
  ...
  async respondToPermission(requestId: string, response: AgentPermissionResponse): Promise<void> {
    ...
    if (isCombinedAskUserPermission(request)) {
      ...
      this.runtimeSession.respondToExtensionUiRequest(
        requestId,
        this.dialect.buildExtensionUiResponse?.(request, response) ??
  ...
  private async requestEntryCapture(reason: string): Promise<void> {
    // D3(omp-native-branch): marker capture stays in shared core for behavior preservation.
    const requestId = randomUUID();
    const resultPromise = this.waitForExtensionResult(requestId);
  ...
  private handleExtensionUiRequest(
    ...
    // D3(omp-native-branch): extension UI permission mapping remains shared for this refactor.
    ...
    const request =
      this.dialect.mapExtensionUiRequestToPermission?.(event) ??
      mapExtensionUiRequestToPermission(event, {
        provider: this.provider,
        label: this.dialect.label,
  ...
  private handleMessageEnd(
    ...
    if (event.message.role === "custom") {
      ...
      const mappedEvent = text ? this.dialect.mapCustomMessage?.(text, this.provider) : null;
      if (mappedEvent) {
        this.emit({ ...mappedEvent, turnId });
      } else if (text) {
        this.emit({
          type: "timeline",
          provider: this.provider,
...
export class PiRpcAgentClient implements AgentClient {
  ...
  constructor(options: PiRpcAgentClientOptions) {
    this.dialect = options.dialect ?? PI_DIALECT;
    this.provider = this.dialect.providerId;
    this.capabilities = capabilitiesForClient(this.dialect);
    this.logger = options.logger;
    this.runtimeSettings = options.runtimeSettings;
    this.providerParams = PiProviderParamsSchema.parse(
      options.providerParams ??
        (this.dialect.defaultSessionDir ? { sessionDir: this.dialect.defaultSessionDir } : {}),
    );
    this.runtime =
      options.runtime ?? createRuntime(options.logger, options.runtimeSettings, this.dialect);
  }
  ...
  async createSession(
    ...
    try {
      return new PiRpcAgentSession({
        ...
        capabilities: capabilitiesForSession(this.dialect, mcpConfig !== null),
  ...
  async fetchCatalog(options: FetchCatalogOptions): Promise<ProviderCatalog> {
    const launchMode = resolveDialectLaunchMode(this.dialect, undefined);
  ...
  async importSession(input: ImportProviderSessionInput, context: ImportProviderSessionContext) {
    const dialectImport = await this.dialect.importSession?.({ client: this, input, context });
    if (dialectImport) {
      return dialectImport;
    }
  ...
  async getDiagnostic(): Promise<{ diagnostic: string }> {
    try {
      ...
      return {
        diagnostic: formatProviderDiagnostic(this.dialect.label, [
      ...
      return {
        diagnostic: formatProviderDiagnosticError(this.dialect.label, error),
      };
    }
  }
  ...
  private async resolvePiLaunch(): Promise<ResolvedProviderLaunch> {
    return resolveProviderLaunch({
      ...
      defaultBinary: this.dialect.defaultCommand[0],
    });
  }
}
