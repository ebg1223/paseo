...
describe("provider child ownership controls", () => {
  it("does not interrupt a provider-owned child", () => {
  ...
  it("rejects sends before writing optimistic state", async () => {
    const sendAgentMessage = vi.fn();
    await expect(
      dispatchComposerAgentMessage({
        client: { sendAgentMessage, uploadFile: vi.fn() },
        agentId: "child",
        text: "hello",
        attachments: [],
        encodeImages: vi.fn(),
    ...
    ).rejects.toThrow("cannot be prompted");
