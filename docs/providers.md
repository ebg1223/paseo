...
OMP is a first-class built-in provider, disabled by default. It shares the Pi runtime seam through `providers/pi-shared/` but owns a dedicated adapter in `providers/omp/`. It launches `omp --mode rpc-ui`, uses OMP's `get_available_commands` RPC for slash-command discovery, bridges OMP `rpc-ui` approval dialogs into Paseo permissions, and imports terminal-started sessions from `~/.omp/agent/sessions` when enabled.
